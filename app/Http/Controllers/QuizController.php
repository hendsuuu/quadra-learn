...
class QuizController extends Controller
{
    ...
    public function show($quizKey)
    {
        ...
        if (!isset($this->quizzes[$quizKey])) {
            abort(404, 'Quiz not found');
        }

        $quiz = $this->quizzes[$quizKey];
    ...
    public function result(Request $request, $quizKey)
    {
        ...
        if ($progress->progress < array_search($quizKey, array_column($this->menus, 'key')) + 1) {
            $progress->progress = array_search($quizKey, array_column($this->menus, 'key')) + 1;
            $progress->save();
        }
    ...
    public function evaluasi(Request $request){
        $answers=AnswerKey::query()->whereIn("code", array_keys($request->all()))->get();
    ...
    public function evaluasiView(){
        $progress = Progress::where('user_id', auth()->user()->id)->first();
        $questions = $this->quizzes['evaluasi']['questions'];
        return view('layouts.quiz', [
            'questions' => $questions,
            'progress' => $progress,
            'menus' => $this->menus,
            'quizKey' => 'evaluasi', // opsional, untuk konsistensi
            'quiz' => [
                'title' => 'Evaluasi',
                'questions' => $questions,
            ],
